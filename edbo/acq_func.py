...
def expected_improvement(model, obj, jitter=0.01, k=1):
    ...
    jitter : float
        Parameter which controls the degree of exploration.
    k : int
        Will compare to the kth highest value
    ...
    if len(obj.results) == 0:
        ...
        if len(obj.results) >= k:
            index = -1 * k
        else:
            index = -1  # Just take the top result
        max_observed = obj.results.sort_values(obj.target).iloc[index]
